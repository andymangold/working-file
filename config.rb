...
data.episodes.each do |episode|
  if episode.published == true
    proxy "/episodes/#{episode.id}.html", "/episodes/template.html", :locals => { :id => episode.id, :date => episode.date, :title => episode.title, :soundcloudID => episode.soundcloudID, :contributorSlugs => episode.contributorSlugs, :showNotes => episode.showNotes, :links => episode.links }
  end
end
